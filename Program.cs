...
// Prioritize appsettings.json connection string, then fallback to environment variables
string connectionString;
string databaseSource = "Unknown";

// First try to get connection string from appsettings.json
connectionString = builder.Configuration.GetConnectionString("DefaultConnection");

if (!string.IsNullOrEmpty(connectionString))
{
    // Using connection string from appsettings.json (Neon Database)
    databaseSource = "Neon Database (appsettings.json)";
    Console.WriteLine($"Using {databaseSource} - Connection string from appsettings.json");
    
    // Add connection pooling and timeout settings to the existing connection string
    if (!connectionString.Contains("Connection Idle Lifetime"))
    {
        connectionString += ";Connection Idle Lifetime=30;Maximum Pool Size=10;Minimum Pool Size=1;Timeout=15;Command Timeout=30;";
    }
}
else
{
    ...
    else
    {
        ...
        if (string.IsNullOrEmpty(connectionString))
        {
            throw new InvalidOperationException("Database connection string not found. Please configure DefaultConnection in appsettings.json or set DATABASE_URL environment variable.");
        }
    }
}
